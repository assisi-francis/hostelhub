...
for file_path in files:
    ...
    # ensure we only append if it's missing (check for "let width = img.width;")
    if "let width = img.width;" not in content:
        parts = content.rsplit('</script>', 1)
        content = parts[0] + missing_js + '\n</script>' + parts[1]
        with open(file_path, 'w') as f:
            f.write(content)

print("Missing JS injected into messages and verification.")
